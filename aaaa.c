...
void imprimi_arreglo(int arreglo[],int largo,int rango){
    int i;
    for (i=0;i<largo;i++){
        cout<<"rango: "<<rango<<": "<<arreglo[i]<<endl;
    }
}
...
{ 
    ...
    while(paso<procesos){
        ...
        if(rango%(2*paso)==0){// verifica si es el vecino izquiero
            ...
            if(rango+paso<procesos){
                ...
                //ahora sub arreglo dubplica su tamaño para contener la union de ambos arreglos
            ...
            //cout<<"rango"<<rango<<" envio a"<<rango-paso<<endl;
            break;
    ...
    if(rango==0){
        cout<<endl<<"al final arreglado"<<endl;
        imprimi_arreglo(sub_arreglo,num_datos,rango);
    }
    delete[]Arreglo;
    delete[]sub_arreglo; 
    MPI_Finalize();
